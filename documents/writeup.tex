...
resolution on the
classification of emotion in video. I build different models to assess each
...
communicate their emotions amongst
...
low resolution images - called pyramid of local binary patterns. Finally, T.
...
A whole video is fed into the network, and each frames then passes through the
...
talking about their performance with the varied framerate and resolutions. I will
...
across different framerates and resolutions (see Table \ref{tab:acc-mean}
...
the one exception to that is the result of the highest resolution/framerate
video, which
...
framerate, and would like to investigate further, however for now a fix
...
Finally, some studies could be done to see if human subjects can perceive the
...
on
8-class classification of the RAVDESS dataset. Additionally, results show that
