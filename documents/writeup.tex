...
a reduced resolution as there is sufficient work already on that topic, for
example \cite{zheng2010face} and \cite{tian2004evaluation}.
...
I tried two different CNN architectures, ResNet18 and ResNet50. They are 18
and 50 layer versions of the same family of Residual Networks, designed by K.
...
learning the task you want. In our case we keep the base of the network the
same and freeze it throughout training, so those weights remain unchanged. 
...
Finally, we
add two dense layers to the head of the network which we do train. 

To train the network we use stochastic gradient descent, and in order to
obtain good hyperparameters  we use Bayesian optimization TODO cite. Bayesian
optimization represents the problem as a probabilistic model, which maps
