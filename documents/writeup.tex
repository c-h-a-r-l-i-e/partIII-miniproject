...
convolves a filter with the pixel data, and can generate meaningful features
by doing this. CNNs have achieved some impressive results in classifcation
challenges TODO: cite. Common architectures have been designed for different
purposes, for example the VGG network and \cite{}<++> and Residual Networks
(ResNets) \cite{}<++>, both of which were able to achieve winning results in
the ImageNet classification challenge \cite{}<++>.

For classifying videos, it is often vital to take temporal data into account,
and as a result Recurrent Neural Networks (RNNs) \cite{}<++> are a good
...
architecture is Long Short Term Memory (LSTM), which make use of gates to
...
We aim to test various classifiers 

\section{Related Work}

\subsection{Facial Expression Recognition for Images}

\subsection{Facial Expression Recognition for Videos}

\subsection{Compensating for Low Resolution}
